...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, obs_shape[0]+self.num), dtype=np.float32)
        ...
        self.hid_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, num_agents, num_agents, self.hidden_size),
            dtype=np.float32)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
               value_preds, rewards, masks, hid_state=None, bad_masks=None, active_masks=None, available_actions=None):
        self.share_obs[self.step + 1] = share_obs.copy()
        self.obs[self.step + 1] = obs.copy()
        self.rnn_states[self.step + 1] = rnn_states_actor.copy()
        self.rnn_states_critic[self.step + 1] = rnn_states_critic.copy()

        self.hid_states[self.step + 1] = hid_state.copy()
    ...
    def after_update(self):
        ...
        self.hid_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, self.num, self.num, self.hidden_size),
            dtype=np.float32)
    ...
    def feed_forward_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        hid_state = self.hid_states[:-1].reshape(-1, *self.hid_states.shape[3:])
        ...
        for indices in sampler:
            ...
            hid_state_batch = hid_state[indices]
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                  ...
                  adv_targ, available_actions_batch, hid_state_batch
    ...
    def naive_recurrent_generator(self, advantages, num_mini_batch):
        ...
        hid_state = self.hid_states(-1, batch_size, *self.hid_states.shape[3:])
        ...
        for start_ind in range(0, batch_size, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                hid_state_batch.append(hid_state[0:1, ind])
            ...
            hid_state_batch = np.stack(hid_state_batch).reshape(N, *self.hid_states.shape[3:])
